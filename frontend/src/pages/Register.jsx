...
export default function Register({ toast }) {
  ...
  async function handleDirector(e) {
    ...
    setLoading(true)
    try {
      await post('/auth/centro', formDir)
      setSuccess(true)
    } catch (err) { setError(err.message) }
    finally { setLoading(false) }
  }
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--black)', display:'flex', alignItems:'center', justifyContent:'center', padding: isMobile ? 16 : 24 }}>
      <div style={{ background:'#fff', borderRadius: isMobile ? 14 : 20, padding: isMobile ? '28px 22px' : 40, maxWidth:520, width:'100%', boxShadow:'0 8px 40px rgba(0,0,0,.3)' }}>
        ...
        <h1 style={{ fontSize: isMobile ? 22 : 24, fontWeight:800, marginBottom:4 }}>Crear cuenta</h1>

        <div style={{ display:'flex', gap:4, marginBottom:20, background:'var(--bg)', borderRadius:10, padding:4, border:'1.5px solid var(--border)', marginTop:16 }}>
          {[
        ...
        {modo === 'profesor' && (
          <form onSubmit={handleProfesor}>
            <div style={{ background:'var(--primary-pale)', border:'1.5px solid #6ee7b7', borderRadius:10, padding:'12px 16px', marginBottom:20, fontSize:13, color:'var(--primary-dark)', display:'flex', alignItems:'flex-start', gap:8 }}>
              <Info size={16} style={{ flexShrink:0, marginTop:1 }} />
              <span>Tu cuenta necesitará aprobación del director antes de poder acceder.</span>
            </div>
            ...
            <button type="submit" className="btn btn-full" disabled={loading} style={{ background:'var(--black)', color:'#fff', fontSize:15, fontWeight:700, display:'inline-flex', alignItems:'center', justifyContent:'center', gap:8 }}>
              {loading ? 'Enviando...' : <>Solicitar acceso <ArrowRight size={16} /></>}
            </button>
          </form>
        )}
        ...
        {modo === 'centro' && (
          <form onSubmit={handleDirector}>
            ...
            <button type="submit" className="btn btn-full" disabled={loading} style={{ background:'var(--black)', color:'#fff', fontSize:15, fontWeight:700, display:'inline-flex', alignItems:'center', justifyContent:'center', gap:8 }}>
              {loading ? 'Creando centro...' : <>Registrar centro <ArrowRight size={16} /></>}
            </button>
          </form>
        )}
