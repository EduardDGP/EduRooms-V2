...
import { LayoutGrid, Shield, Users, Bell, MessageSquare, BookOpen, Clock, CheckCircle, ChevronDown, ArrowRight, Zap, Menu, X } from 'lucide-react'
...
function FAQ({ q, a }) {
  ...
  return (
    <div style={{ borderBottom:'1px solid rgba(255,255,255,.08)' }}>
      <button onClick={() => setOpen(o => !o)} style={{ width:'100%', display:'flex', alignItems:'center', justifyContent:'space-between', gap:16, padding:'22px 0', background:'transparent', border:'none', cursor:'pointer', fontFamily:'inherit', textAlign:'left' }}>
...
export default function Landing() {
  ...
  const PAD_X = isMobile ? 20 : 40

  return (
    <div style={{ background:'#080808', minHeight:'100vh', fontFamily:"'Outfit', sans-serif", color:'#fff', overflowX:'hidden' }}>

      {/* Navbar */}
      <nav style={{ position:'fixed', top:0, left:0, right:0, zIndex:100, padding:`0 ${PAD_X}px`, height:64, display:'flex', alignItems:'center', justifyContent:'space-between', background: scrollY > 40 || menuOpen ? 'rgba(8,8,8,.92)' : 'transparent', backdropFilter: scrollY > 40 || menuOpen ? 'blur(20px)' : 'none', borderBottom: scrollY > 40 ? '1px solid rgba(255,255,255,.06)' : 'none', transition:'all .3s' }}>
        ...
        {isMobile && (
          <button onClick={() => setMenuOpen(o => !o)} aria-label={menuOpen ? 'Cerrar menú' : 'Abrir menú'}
            style={{ width:40, height:40, border:'none', background:'rgba(255,255,255,.06)', color:'#fff', borderRadius:8, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
      ...
      {isMobile && menuOpen && (
        <div style={{ position:'fixed', top:64, left:0, right:0, bottom:0, zIndex:99, background:'rgba(8,8,8,.98)', backdropFilter:'blur(20px)', padding:'32px 24px', display:'flex', flexDirection:'column', gap:4 }}>
          <a href="#funcionalidades" onClick={closeMenu} style={{ color:'#fff', textDecoration:'none', fontSize:18, fontWeight:600, padding:'16px 0', borderBottom:'1px solid rgba(255,255,255,.08)' }}>Funcionalidades</a>
          <a href="#precio" onClick={closeMenu} style={{ color:'#fff', textDecoration:'none', fontSize:18, fontWeight:600, padding:'16px 0', borderBottom:'1px solid rgba(255,255,255,.08)' }}>Precio</a>
          <a href="#faq" onClick={closeMenu} style={{ color:'#fff', textDecoration:'none', fontSize:18, fontWeight:600, padding:'16px 0', borderBottom:'1px solid rgba(255,255,255,.08)' }}>FAQ</a>
          <Link to="/login" onClick={closeMenu} style={{ color:'#fff', textDecoration:'none', fontSize:18, fontWeight:600, padding:'16px 0', borderBottom:'1px solid rgba(255,255,255,.08)' }}>Iniciar sesión</Link>
          <Link to="/registro" onClick={closeMenu} style={{ marginTop:20, background:'#34d399', color:'#0a0a0a', textDecoration:'none', fontSize:16, fontWeight:700, padding:'14px 20px', borderRadius:10, textAlign:'center' }}>Registrar centro</Link>
        </div>
      )}

      {/* Hero */}
      <section style={{ minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:`120px ${PAD_X}px 80px`, textAlign:'center' }}>
        ...
        <h1 style={{ fontSize:'clamp(36px, 7vw, 76px)', fontWeight:900, lineHeight:1.05, letterSpacing:'-2px', marginBottom:24, opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(30px)', transition:'opacity .8s 150ms, transform .8s 150ms' }}>
          Tu centro educativo,<br /><span style={{ color:'#34d399' }}>organizado al instante</span>
        </h1>
        <p style={{ fontSize: isMobile ? 17 : 20, color:'rgba(255,255,255,.55)', lineHeight:1.7, maxWidth:580, margin:'0 auto 48px', opacity: heroVisible ? 1 : 0, transform: heroVisible ? 'translateY(0)' : 'translateY(30px)', transition:'opacity .8s 300ms, transform .8s 300ms' }}>
      ...
      {/* Funcionalidades */}
      <section id="funcionalidades" style={{ padding:`100px ${PAD_X}px`, maxWidth:1100, margin:'0 auto' }}>
        <div style={{ textAlign:'center', marginBottom:64 }}>
          ...
          <h2 style={{ fontSize:'clamp(28px, 5vw, 52px)', fontWeight:800, letterSpacing:'-1.5px' }}>Todo lo que necesita tu centro</h2>
        </div>
      ...
      {/* Ahorro de tiempo */}
      <section style={{ padding:`100px ${PAD_X}px`, background:'rgba(255,255,255,.02)', borderTop:'1px solid rgba(255,255,255,.06)', borderBottom:'1px solid rgba(255,255,255,.06)' }}>
        <div style={{ maxWidth:1000, margin:'0 auto' }}>
          <div style={{ textAlign:'center', marginBottom:64 }}>
            ...
            <h2 style={{ fontSize:'clamp(28px, 5vw, 52px)', fontWeight:800, letterSpacing:'-1.5px' }}>Menos papeleo, más docencia</h2>
          </div>
          <div ref={savingsRef} style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(min(100%, 220px), 1fr))', gap:20 }}>
            {[
          ...
          <p style={{ textAlign:'center', fontSize: isMobile ? 16 : 18, color:'rgba(255,255,255,.6)', lineHeight:1.8, marginTop:40 }}>
      ...
      {/* Precio */}
      <section id="precio" style={{ padding:`100px ${PAD_X}px`, maxWidth:800, margin:'0 auto', textAlign:'center' }}>
        <p style={{ color:'#34d399', fontSize:13, fontWeight:700, textTransform:'uppercase', letterSpacing:'2px', marginBottom:12 }}>Precio</p>
        <h2 style={{ fontSize:'clamp(28px, 5vw, 52px)', fontWeight:800, letterSpacing:'-1.5px', marginBottom:16 }}>Simple y transparente</h2>
        <p style={{ color:'rgba(255,255,255,.5)', fontSize:16, marginBottom:56 }}>Sin sorpresas. Un precio para todo tu centro.</p>
        <div style={{ background:'rgba(255,255,255,.03)', border:'1px solid rgba(52,211,153,.3)', borderRadius:24, padding: isMobile ? '36px 24px' : '48px', position:'relative', overflow:'hidden' }}>
          ...
          <div style={{ fontSize: isMobile ? 56 : 72, fontWeight:900, letterSpacing:'-3px', lineHeight:1 }}>30<span style={{ fontSize: isMobile ? 26 : 32, color:'rgba(255,255,255,.4)', fontWeight:500 }}>€</span></div>
          <div style={{ color:'rgba(255,255,255,.5)', fontSize:16, marginBottom:40 }}>por centro / mes · IVA no incluido</div>
          <div style={{ display:'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap:14, textAlign:'left', marginBottom:40 }}>
      ...
      {/* FAQ */}
      <section id="faq" style={{ padding:`80px ${PAD_X}px`, maxWidth:700, margin:'0 auto' }}>
        <div style={{ textAlign:'center', marginBottom:56 }}>
          <p style={{ color:'#34d399', fontSize:13, fontWeight:700, textTransform:'uppercase', letterSpacing:'2px', marginBottom:12 }}>FAQ</p>
          <h2 style={{ fontSize:'clamp(24px, 4vw, 44px)', fontWeight:800, letterSpacing:'-1px' }}>Preguntas frecuentes</h2>
        </div>
      ...
      {/* CTA Final */}
      <section style={{ padding:`100px ${PAD_X}px`, textAlign:'center' }}>
        <div ref={ctaRef} style={{ opacity: ctaVisible ? 1 : 0, transform: ctaVisible ? 'translateY(0)' : 'translateY(40px)', transition:'opacity .8s, transform .8s' }}>
          <h2 style={{ fontSize:'clamp(32px, 6vw, 64px)', fontWeight:900, letterSpacing:'-2px', lineHeight:1.05, marginBottom:24 }}>
            Puedes registrarte<br /><span style={{ color:'#34d399' }}>ahora mismo</span>
          </h2>
          <p style={{ color:'rgba(255,255,255,.5)', fontSize: isMobile ? 16 : 18, marginBottom:48, lineHeight:1.7 }}>
      ...
      {/* Footer */}
      <footer style={{ borderTop:'1px solid rgba(255,255,255,.06)', padding: isMobile ? '28px 20px' : 40, display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:16 }}>
        <span style={{ fontSize:15, fontWeight:700 }}>Ex<span style={{ color:'#34d399' }}>Rooms</span></span>
        <p style={{ color:'rgba(255,255,255,.25)', fontSize:13, order: isMobile ? 3 : 0, width: isMobile ? '100%' : 'auto', textAlign: isMobile ? 'center' : 'left' }}>© 2026 ExRooms · Gestión interna para centros educativos</p>
